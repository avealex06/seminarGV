        ...
        static List<int> DepthFirstSearch(BinaryTree graph, int targetNodeIndex)
        {
            ...
            while (true)
            {
                if (currentNode.index != targetNodeIndex)
                {
                    ...
                    {
                        if (currentNode.visitedChildren == null)
                        {
                            currentNode = currentNode.children[0];
                            pathToTargetNode.Add(currentNode.index);
                        }
                        else if (currentNode.visitedChildren.Count !> currentNode.children.Count)
                        {
                            
                            currentNode = currentNode.children[currentNode.visitedChildren.Count];
                            pathToTargetNode.Add(currentNode.index);
                        }
                        else
                        {
                            Node previousNode = currentNode.parent;
                            pathToTargetNode.Add(previousNode.index);
                            currentNode = previousNode.children[currentNode.index + 1];
                            pathToTargetNode.Add(currentNode.index);
                        }
