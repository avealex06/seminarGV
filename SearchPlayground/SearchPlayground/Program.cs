    ...
    internal class Program
    {
        static int LinearSearch(int[] array, int elementToSearch)
        {
            ...
            foreach (int num in array)
            {
                ...
                {
                    Console.WriteLine("found element at index " + Array.IndexOf(array, num));
                }
            }
            return -1;
        }
        ...
        static int BinarySearch(int[] array, int elementToSearch)
        {
            ...
            int topIndex = array.Length;
            int bottomIndex = 0;
            int midIndex = topIndex / 2;
            while (midIndex!=elementToSearch)
            {
                if (midIndex<elementToSearch)
                {
                    bottomIndex = midIndex;
                    midIndex = (bottomIndex + topIndex) / 2;
                }
                else if (midIndex>elementToSearch)
                {
                    topIndex = midIndex;
                    midIndex = topIndex / 2;
                }
            }
            Console.WriteLine("found element at index " + midIndex);
